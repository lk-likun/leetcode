...
def link(num, path_ls):
    history = dict()
    for i in path_ls:
        tmp_right = history.get(i[0], list())
        tmp_right.append(i[1])
        tmp_left = history.get(i[1], list())
        tmp_left.append(i[0])
        history.update({i[0]: tmp_right, i[1]: tmp_left})
    if not history:
    ...
    for j in range(num):
        ...
        while queue[0]:
            ...
            tmp_ls = list()
            for z in tmp:
                next_v = history.get(z)
                v = copy.deepcopy(next_v)
                for h in past:
                    if h in v:
                        v.remove(h)
                tmp_ls.extend(v)
